...
export default function PageSection({ id, title, subtitle, titleIcon: Icon, children }: Props & {children: React.ReactNode}) {
  ...
  return (
    ...
    initial={{ opacity: 0, y: 20 }}              // Hidden and shifted down initially
      ...
      ref={ref}
      id={id}
      className='min-h-screen flex flex-col justify-center text-white bg-slate-900 py-12'
    >
        <div className={`${Icon ? 'flex' : ''} px-8 py-10 gap-2 `}>
            {Icon && <div> <Icon className=" h-full w-12 " /> </div>}
            <div>
                {subtitle && <p className={`font-inter uppercase tracking-wider text-md md:text-xl font-bold text-cyan-400 `}>{subtitle}</p>}
                {title && <h1 className="text-3xl md:text-6xl font-bold font-display mb-4 text-pink-500">{title}</h1>}
            </div>
        </div>
